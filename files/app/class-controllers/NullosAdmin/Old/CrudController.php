                    ...
                    if ('formPost' === $type || "ajaxFormPost" === $type) {
                        ...
                        if (array_key_exists('ric', $_POST)) {
                            ...
                            if (false !== ($values = $prc->readByRic($ric))) {
                                $values = $this->simplifyFormValues($values);
...
//                            ->setJsInitScript("/path");
    ...
    /**
     * In nullos, the crud system with list works with column names prefixed with the table names.
     * However, for forms, since the "nullos" crud system only uses one form at the time,
     * we "know" that we don't need foreign fields, hence we drop the table prefix.
     *
     */
    private function simplifyFormValues(array $values)
    {
        $ret = [];
